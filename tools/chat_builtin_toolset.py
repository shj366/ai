...
def build_chat_builtin_capability() -> AbstractCapability[ChatAgentDeps]:
    ...
    async def get_database_schema(
        ...
        def run_inspect(connection):
            ...
            for table_name in table_names:
                if table_name not in all_tables:
                    schema_info[table_name] = 'Table not found.'
                    continue
        ...
        try:
            return await conn.run_sync(run_inspect)
    ...
    async def execute_sql_query(
        ...
        try:
            ...
            if len(rows) > 2000:
                ...
                warning = (
                    'Result set exceeded 2000 rows limit. Results have been truncated. '
                    'Please use aggregation (SUM, COUNT, GROUP BY) or LIMIT in your SQL query for large datasets.'
                )
            else:
                rows_to_return = rows
                warning = None
            ...
            response = {'data': [dict(row) for row in rows_to_return]}
            if warning:
                response['warning'] = warning

            return json.dumps(response, default=default_serializer, ensure_ascii=False)
        except Exception as e:
            return json.dumps({'error': f'SQL Execution Error: {e}'}, ensure_ascii=False)
    ...
    async def download_attachments(
        ...
        重要说明：
        - 此工具返回 JSON 格式的下载命令，前端会自动解析并触发下载。
        - 返回格式：{"action": "download_attachments", "entity_type": "company_qualification", "entity_ids": [1, 2, 3]}
        - 前端会识别 action 字段并自动调用下载 API。

        Args:
            entity_type: 实体类型，例如："company_qualification", "intellectual_property_patent", "intellectual_property_trademark", "intellectual_property_software", "staff_credential"
            entity_ids: 实体 ID 列表
        ...
        return json.dumps(
            {
                ...
                'message': (
                    f'prepared to download {len(entity_ids)} {entity_type} attachment files for you, '
                    'starting download...'
                ),
            },
            ensure_ascii=False,
